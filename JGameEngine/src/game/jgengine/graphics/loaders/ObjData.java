...
public class ObjData
{
	...
	{
		//nombre de vertexs: faces.size() * 3
		//taille vertex: 3 positions + 2 text coords
		float[] vertices = new float[faces.size() * (3 + 2)]; //sans compter les normales
		...
		for(Vector3i face : faces)
		{
			...
			//System.out.println("POS: " + positions.get(vs[j].x - 1).x + " " + positions.get(vs[j].x - 1).y + " " + positions.get(vs[j].x - 1).z);
			vertices[i++] = textureCoords.get(face.y - 1).x;
			vertices[i++] = textureCoords.get(face.y - 1).y;
			//System.out.println("TC: " + textureCoords.get(vs[j].y - 1).x + " " + textureCoords.get(vs[j].y - 1).y);
		}
