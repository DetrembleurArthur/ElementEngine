...
public class Window
{
	...
	private final Vector2f monitorSize;
	private final float targetApectRatio;
	private Vector2f aspectRatioSize;
	public static Window WINDOW = null;
	private boolean sizeRatioEnable;
	private Vector2f aspectRatioPosition;
	...
	public Vector2f getAspectRatioPosition()
	{
		return aspectRatioPosition;
	}
	...
	public Window(int width, int height, String title) throws SysException
	{
		...
		aspectRatioPosition = new Vector2f();
		aspectRatioSize = new Vector2f(width, height);
	}
	...
	public void aspectRatioUpdateViewport(float w, float h)
	{
		...
		aspectRatioSize = new Vector2f(aspectWidth, aspectHeight);

		trueRatio = aspectRatioSize.x / firstSize.x;

		glViewport(vpx, vpy, aspectWidth, aspectHeight);
		if(sizeRatioEnable)
		{
			glfwSetWindowSize(windowId, (int) aspectWidth, (int) aspectHeight);
			aspectRatioPosition = new Vector2f(0, 0);
		}
		else
		{
			glfwSetWindowSize(windowId, (int) w, (int) h);
			aspectRatioPosition = new Vector2f(vpx, vpy);
		}
	}
